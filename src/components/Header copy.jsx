...
function Header({activeColor,activebg,menutextt,preLoaderOut}) {
...
// const rotation = useTransform(scrollMarker, [0, 1], [30, 3360]);
  ...
  return (
    <div className="mix-blend-differenc fixed top-[3%] w-full z-[50]">
      <header
        ...
        ref={ref}
        className="z-[60]  container px-5 mx-auto flex justify-center items-center"
      ...
      transition={{duration:0.4,delay:0}}
        className=" bg-red600 shadowlg backdropblur-lg  roundedfull w-full flex items-center justify-between py-2">
          ...
          <ul className= " hidden md:flex flex-row gap-6 mx-auto">
           ...
           {/* <Magnetic> */}
            ...
            <div className="relative">
              <motion.div variants={menu} animate={burgerMenu?'opened':'closed'} initial="closed" className="absolute right-0 rounded-[1.2em] bg-red-600 w-[300px] h-[550px]">
      ...
      <motion.div variants={scaleMenu} initial='initial' animate={preLoaderOut?'enter':'initial'} exit={'exit'} ref={scaleMenuIcon}
       onClick={(e)=>{setBurgerMenu(!burgerMenu)}}
        className=" w-[70px] h-[70px] lg:w-[80px] lg:h-[80px] absolute top-0 right-0 cursor-pointer flex items-center justify-center ">     
  <motion.div
  ...
  >
    ...
    <motion.h2 className={` cursor-pointer flex items-center justify-center bg-slate500 w-full pointer-events-none relative z-10 text-[16px] font-custom uppercase font-bold text-center`} animate={{ color: menutextt }}
      transition={{ duration: 0.6, ease: "easeInOut" }}>
    {/* Menu */}
  {/* <MenuIcon/> */}
    <motion.div animate={{ color: menutextt, }}
     className={clsx(
            "before:bg-slate-600 after:bg-slate-600  burger text-black",
